            ...
            try {
              ...
              if(timerName.length() <= MAX_NAME_LENGTH) {
                if(!Util.timers.containsKey(timerName)) {
                  ...
                  this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.created", TextColor.color(170, 170, 170),
                  ...
                  if(Util.timers.size() > WARNING_AMOUNT & !warningSent) {
                    warningSent = true;
                    this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.performance", TextColor.color(255, 85, 85))));
                  }
                } else {
                  this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.already-exists", TextColor.color(255, 85, 85))));
                }
              } else {
                this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.max-length", TextColor.color(255, 85, 85),
                    Component.text(MAX_NAME_LENGTH))));
              }
            } catch (NumberFormatException ignored) {
              this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.no-number", TextColor.color(255, 85, 85))));
            }
          } else {
            this.displayMessage(this.addon.prefix.copy().append(Component.translatable("moneymaker.command.timer.usage", TextColor.color(255, 85, 85))));
          }
        } else {
